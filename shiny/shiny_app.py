...
app_ui = ui.page_fluid(
    ...
    ui.panel_conditional("output.middleHE=='MSI H&E image detected'",
            ...
            # Show recorded coordinates in table
)
 #   ),
#    ui.layout_columns(
#        ui.card(ui.output_plot("plot_noHE_left", click=True)),  # Enable click on plot
...
#            ui.panel_conditional(
#                "input.plot_noHE_left_click", 
#                ui.output_plot("plot_noHEwithselected_left", click=False))),
#        ui.card(
#            ui.panel_conditional(
#                "input.plot_noHE_right_click", 
#                ui.output_plot("plot_noHEwithselected_right", click=False)))),
#    ui.h3("Recorded Landmarks"),
#    ui.output_table("coords")            # Show recorded coordinates in table
...
#    @render.table
    ...
    # Function to update the plot and capture clicks
    @output
    @render.plot
    @reactive.event(input.run_dimred)
    def plot_noHE_left():
        # Load the images
        fig,axs = msi_dimred()
 #       msi_image = imread(table['MSI_HE_image'][int(input.pick_sample())])
        return fig
    ...
    def plot_noHE_withselected_left():
        ...
        current_coords_left = clicked_coords_noHE_left()
    ...
    def plot_noHE_withselected_right():
        ...
        # Get the list of clicked coordinates
        current_coords_right = clicked_coords_noHE_right()
    ...
    @output
    @render.table
    @reactive.event(input.plot_noHE_left_click,input.plot_noHE_right_click)
    def coords_noHE():
...
#     # Display the clicked coordinates as a table
    @output
    @render.table
    @reactive.event(input.plot_MSI2HE_left_click,input.plot_MSI2HE_right_click)
    def coords_MSI2HE():
        # Retrieve the stored coordinates
        current_coords_left = clicked_coords_MSI2HE_left.get()
        current_coords_right = clicked_coords_MSI2HE_right.get()
...
#     # Display the clicked coordinates as a table
    @output
    @render.table
    @reactive.event(input.plot_HE2HE_left_click,input.plot_HE2HE_right_click)
    def coords_HE2HE():
        # Retrieve the stored coordinates
        current_coords_left = clicked_coords_HE2HE_left.get()
        current_coords_right = clicked_coords_HE2HE_right.get()
